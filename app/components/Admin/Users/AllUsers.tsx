...
const AllUsers: FC<Props> = ({ isTeam }) => {
  ...
  const [
    updateUserRole,
    { error: updateUserError, isSuccess: updateUserIsSuccess },
  ] = useUpdateUserRoleMutation();
  ...
  const [
    deleteUser,
    { error: deleteUserError, isSuccess: deleteUserIsSuccess },
  ] = useDeleteUserMutation({});

  useEffect(() => {
    if (updateUserError) {
      if ("data" in updateUserError) {
        const errorMessage = updateUserError as any;
    ...
    if (deleteUserIsSuccess) {
      refetch(); // call to update redux state
            ...
            >
              <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[450px] bg-white dark:bg-slate-900 rounded-[8px] shadow p-4 outline-none">
                ...
                <select
                  name=""
                  id=""
                  className={`${styles.input} !mt-6 dark:text-white dark:bg-black`}
